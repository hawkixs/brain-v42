...
This module used to live at ``scripts/dream/_agent_capability.py``, private to
the Dream orchestrator and outside the wheel. It is ported here UNCHANGED
(lot 1 of the agent runtime extraction, Brain ticket c31bad72) so a second
consumer -- the extract rail today, a PR reviewer service tomorrow -- can
depend on the package instead of growing a third copy. ``scripts/dream``
depends on the package now; the package depends on nothing under ``scripts/``.

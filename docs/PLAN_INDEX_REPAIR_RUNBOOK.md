...
| Recovery contract, repository target | `ops/recovery/brain-v42-v9.sql` | **NOT MEASURED against production.** Minted 2026-09-03 for head `052`, verified 22/30 on a disposable database built by the alembic chain (the eight failures are six data checks on an empty database, the server's vector build, and 050's disabled trigger) and 4/4 on the yardstick. Production is still at `051`: v9 gets its live receipt in the apply window, not before. |
| Recovery contract, live target | `ops/recovery/brain-v42-v8.sql` | 2026-09-03, live production. **Stays the asset to replay until 052 is applied** — v9 expects a table production does not have yet, so replaying it now would fail for the right reason and be read as the wrong one. |

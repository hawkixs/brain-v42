...
- The nightly run applies whatever `WET_APPLYABLE_OPS` names. This plan was written when that was `archive`/`status`; the evening of 2026-07-04 widened it to all four ops, merge and rename included (aggressive regime, operator decision, recorded in the constant's comment and pinned by `test_roadmap_curate.py::test_wet_applyable_ops_includes_all_ops`). The listings further down still show the original narrow recipe — they are what the plan asked for, not what runs. `tests/unit/test_roadmap_wet_scope_matches_its_doc.py` keeps this bullet and the rollout step honest.
...
- [ ] **Step 6: Rollout** — ≥2 clean dry nights → review the proposals in the morning: `SELECT id, op, feature_id, payload, rationale FROM roadmap_curation_proposals WHERE status='proposed' ORDER BY id;` → `uv run python -m scripts.roadmap_curate --apply-ids "…"` for the good ones, `UPDATE roadmap_curation_proposals SET status='rejected' WHERE id IN (…);` for the bad ones. When the acceptance rate is stable and high: consider `BRAIN_DREAM_ROADMAP_DRY_RUN=false` — and read what that arms before doing it: since 2026-07-04 `WET_APPLYABLE_OPS` is all four ops, so the flip lets the nightly apply `merge`, `rename`, `archive` and `status` without review. It is NOT the archive/status-only mode this plan originally described. Evidence against flipping today: on 2026-09-02 human review rejected 14 brain-v42 proposals out of 14, all title rewrites (decision `892c1491`); the standing decision is to stay DRY.

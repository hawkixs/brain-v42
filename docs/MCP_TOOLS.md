...
- `at least N candidates, none above min_score X (best raw score Y; threshold applies to the raw score, before decay)` — candidates existed; lower `min_score`. `N` is a FLOOR (`candidates_before_threshold` is capped by the hybrid fan-out's `fused[:20]` and, when `limit < 20`, further by `limit`) — never read it as the true candidate count.
...
When `project_group` resolves to one or more `project_keys`, the scope names both the group and what it resolved to: `project group "red-triad" → proj-a, proj-b`, instead of the plain `project: …` line. An UNRESOLVED `project_group` (matched zero projects) short-circuits the whole explanation to `project group "<name>" → no known project` — the fan-out never ran, so no other counter in that case is a real measurement.

A non-nominal `rerank_mode` (`rrf_fallback` / `rrf_only`) is appended to the explanation when relevant — except when a degraded banner is already rendered above the block (same source, `rerank_mode_observed`; the banner alone carries it, avoiding a two-language duplicate). This block is derived from `SearchResponse.diagnostics` / `WhatDoIKnowResponse.diagnostics` (candidates before threshold, best raw score, tags removed, requested vs. effective project_key and min_score, rerank mode, degraded flag) — the same 7 fields (`candidates_before_threshold`, `best_raw_score`, `tags_filtered_out`, `rerank_mode`, `degraded`, `min_score_effective`, `project_key_effective`) also land in both telemetry events.
...
The repository migration target is migration 057; this does not enable delivery operations in MCP.

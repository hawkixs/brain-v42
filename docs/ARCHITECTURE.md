...
```
 ...
 | 63 always-on + 2 graph-gated = 65 |  | 127.0.0.1:9200       |  | 127.0.0.1:9201       |
...
### 42 PG tables (`src/brain_v42/db/tables.py`)
...
| PROMOTE | `gpt-5.6-sol` / high | `brain_get`, `brain_search`, `brain_propose_adr`, `brain_promote_adr`, `brain_create_runbook`, `brain_promote_runbook`, `brain_list`, `brain_get_neighbors`, `brain_graph_path` |
...
│       └── tools/                # 63 always-on + 2 graph-gated = 65
├── alembic/versions/             # migrations 001 .. 053 defined in the repository
...
| MCP tools | 21 | 63 always-on + 2 graph-gated = 65 |

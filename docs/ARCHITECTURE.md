...
**Updated:** 2026-07-24
**Repository and production state:** migrations 001–052 defined, 33 PG tables modeled; MCP catalog: 54 always-on + 2 graph-gated = 56. Production runs lifecycle v4 since 24 July 2026: revision 036 was applied and validated first, then 037 was proved before the restart-last MCP cutover and authenticated lifecycle-v4 E2E. The deployed Alembic head has since advanced and is not asserted here — measure it with `select version_num from alembic_version`. Last measurement: `045` on 16 August 2026, right after the 044→045 cutover.
...
```
 ...
 | 54 always-on + 2 graph-gated = 56 |  | 127.0.0.1:9200       |  | 127.0.0.1:9201       |
...
│       └── tools/                # 54 always-on + 2 graph-gated = 56
...
| MCP tools | 21 | 54 always-on + 2 graph-gated = 56 |

...
**Updated:** 2026-07-24
**Repository and production state:** migrations 001–053 defined, 42 PG tables modeled; MCP catalog: 62 always-on + 2 graph-gated = 64. Production runs lifecycle v4 since 24 July 2026: revision 036 was applied and validated first, then 037 was proved before the restart-last MCP cutover and authenticated lifecycle-v4 E2E. The deployed Alembic head has since advanced and is not asserted here — measure it with `select version_num from alembic_version`. Last measurement: `045` on 16 August 2026, right after the 044→045 cutover.
...
```
 ...
 | 62 always-on + 2 graph-gated = 64 |  | 127.0.0.1:9200       |  | 127.0.0.1:9201       |
...
| PROMOTE | `gpt-5.6-sol` / high | `brain_get`, `brain_search`, `brain_propose_adr`, `brain_promote_adr`, `brain_create_runbook`, `brain_list`, `brain_get_neighbors`, `brain_graph_path` |
...
│       └── tools/                # 62 always-on + 2 graph-gated = 64
...
| MCP tools | 21 | 62 always-on + 2 graph-gated = 64 |

-- Jumeau -pgrestore de la preuve ACL — la parité RESTAURÉE (ticket ac7b3a49).
--
-- La dérogation v5 (« pas de jumeau : le restore de bac à sable tourne
-- --no-owner --no-acl ») était AUTO-RÉFÉRENTIELLE : ces drapeaux n'existent
-- que dans la phrase qui la justifie. La seule commande pg_restore du runbook
-- (--exit-on-error --clean --create, via le service de maintenance) ne les
-- porte pas. MESURÉ le 2026-08-29 sur un banc jetable PostgreSQL 16.14 :
-- cette commande, jouée sur un dump réel avec les rôles pré-créés, REJOUE
-- l'intégralité des propriétaires et des GRANTs — contract_grant_mismatches=0,
-- relation_owner_mismatches=0, unexpected_grantee_mismatches=0. La prémisse
-- de la dérogation est fausse pour le chemin de SINISTRE ; elle reste vraie
-- pour la RÉPÉTITION sandbox (plan 2026-07-11, --no-owner --no-acl), qui
-- n'est pas la cible de ce jumeau.
--
-- UNE différence dite, jamais tue : le cluster de restauration porte le rôle
-- superuser de son service de maintenance (mesuré : `postgres` sur le banc).
-- Le recensement des rôles tolère donc les rôles SUPERUSER surnuméraires et
-- les NOMME dans le reçu ; un rôle NON-superuser inattendu reste un échec —
-- c'est lui qui signalerait un plan de données surprenant, pas l'outillage
-- du sinistre.
...
role_privilege_mismatches AS (
 SELECT (
     ...
     -- Rôles surnuméraires : un cluster de restauration porte le superuser de
     -- son service de maintenance. Toléré ET nommé (tolerated_superuser_roles) ;
     -- un rôle non-superuser inattendu reste un échec.
     SELECT count(*)
...
check_rows(id, expected, observed, passed) AS (
 SELECT
     ...
     jsonb_build_object(
         ...
         'tolerated_superuser_roles', (
             SELECT COALESCE(jsonb_agg(role_record.rolname ORDER BY role_record.rolname), '[]'::jsonb)
             FROM pg_catalog.pg_roles AS role_record
             LEFT JOIN expected_roles AS expected_role
               ON expected_role.role_name = role_record.rolname
             WHERE role_record.rolname NOT LIKE 'pg\_%'
...
SELECT jsonb_build_object(
 ...
 'contract_id', 'brain-v42/postgresql-recovery/v10-acl-pgrestore',
 'schema_version', 10
)::text
FROM check_rows;

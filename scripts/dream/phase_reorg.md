...
- **This phase is archive-only.** The only mutations it may ever perform, in any run, are the two already described above: normalising or flagging via `tags` (Parts 1 and 3) and setting `freshness_status="archived"` via `brain_update` (Part 2). It never merges or deduplicates entities (the `brain_merge_entities` tool) and never deletes (the `brain_delete` tool) — see the next bullet for the condition that makes both unreachable. It never changes content or `status` fields and never un-archives — both refused by the server by field name, see the last bullet — and never `project_key` or any other ownership field, which the server refuses by name when that same condition holds. Archiving stays reversible, as Part 2 already states — this guard is about what one run may do, not about making the state permanent.
...
- **Restricting writes to `tags` and `freshness_status`, and forbidding un-archival, is enforced while capability enforcement is armed — by the `brain_update` project policy, not by `reorg_validate.py`.** Since ticket e78409da, `PROJECT_TOOL_POLICIES["brain_update"]` carries a per-phase field rule (`phase_update_field_rules["reorg"]`, `src/brain_v42/services/dream_project_scope.py`): a `fields` argument naming anything but `tags` or `freshness_status` — `topic`/`insight` on a learning, `status`/`description`/`reasoning` on a decision — is refused whole-call with reason `field_not_allowed_for_phase`, and `freshness_status` set to anything but `"archived"` (`"fresh"`, `"stale"`) is refused with reason `unarchive_forbidden_for_phase`. Both refusals run inside `DreamCapabilityMiddleware.on_call_tool`, right after the ownership-field refusal above, and stop firing on the documented enforcement-off rollback exactly like it. `reorg_validate.py` still only checks, after the fact, that a declared `archived` id carries `freshness_status='archived'` in Postgres and that a declared `updated` id's tags differ from the pre-phase snapshot (the G4 tags-before/after control) — it never checks that any other field was left untouched, and no longer needs to.

...
def test_stored_contract_revision_requires_normalized_repository_identity_and_is_immutable() -> (
    ...
    payload["deliverables"][0]["repository_id"] = 42
    revision = ContractRevision.model_validate(
        {
            **payload,
            "ticket_id": str(uuid4()),
            "contract_revision": 1,
...
def test_contract_digest_excludes_revision_metadata_and_keeps_contract_content() -> None:
    ...
    payload = contract_payload()
    payload["deliverables"][0]["repository_id"] = 42
    common = {**payload, "content_digest": "c" * 64, "author_project": "brain-v42"}
    first = ContractRevision.model_validate(
        {**common, "ticket_id": str(UUID(int=1)), "contract_revision": 1}
    )
    second = ContractRevision.model_validate(
        {
            **common,
            "ticket_id": str(UUID(int=2)),
            "contract_revision": 2,
            "amendment_reason": "Acceptance criteria clarified",
        }
    )
